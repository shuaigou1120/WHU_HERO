...
//    }
    ...
    while (1)
    {
		...
		data[0]=0;
		data[1]=chassis_move.chassis_relative_angle;
...
//            }
			...
			if(chassis_move.chassis_mode == CHASSIS_VECTOR_RAW)
			{
			    ...
			    //CAN_cmd_gimbal(0,0,0,0);
					//CAN_cmd_chassis(0, 0, 0, 0);
//			if(chassis_move.chassis_mode == CHASSIS_VECTOR_FOLLOW_GIMBAL_YAW||chassis_move.chassis_mode ==CHASSIS_VECTOR_NO_FOLLOW_YAW)
//			{
//				CAN_cmd_chassis(Limit_Chassis_Motor1_Speed_out,Limit_Chassis_Motor2_Speed_out,Limit_Chassis_Motor3_Speed_out,Limit_Chassis_Motor4_Speed_out);
//			}
...
static void chassis_feedback_update(chassis_move_t *chassis_move_update)
{
	...
	chassis_move_update->chassis_relative_angle=angle_change((chassis_move_update->gimbal_yaw_motor.gimbal_motor_measure->ecd)*360/8192);
...
static void chassis_set_contorl(chassis_move_t *chassis_move_control)
{
	...
	fp32 vx1_set=0.0f,vy1_set=0.0f;static  fp32 angle;
...
//        chassis_move_control->vy_set = vy_set;
        ...
        //设置控制相对云台角度
        chassis_move_control->chassis_relative_angle_set =-12;
        //calculate ratation speed
        //计算旋转PID角速度
//        chassis_move_control->wz_set=0;
		if(chassis_move_control->chassis_relative_angle<-10 && chassis_move_control->chassis_relative_angle > -14 )
		{
			chassis_move_control->wz_set=0; 
		}
		else
...
//        chassis_move_control->wz_set = angle_set;
		chassis_move_control->wz_set =12;
//        chassis_move_control->vx_set = fp32_constrain(vx_set, chassis_move_control->vx_min_speed, chassis_move_control->vx_max_speed);
//        chassis_move_control->vy_set = fp32_constrain(vy_set, chassis_move_control->vy_min_speed, chassis_move_control->vy_max_speed);
		angle=(chassis_move_control->chassis_relative_angle+6)/180*3.141592654;
		vx1_set=vx_set*cos(angle)-vy_set*sin(angle);
		chassis_move_control->vx_set=fp32_constrain(vx1_set,chassis_move_control->vx_min_speed, chassis_move_control->vx_max_speed);
		vy1_set=vx_set*sin(angle)+vy_set*cos(angle);
...
//        chassis_move_control->wz_set = angle_set;
	...
	if(((chassis_move.chassis_data_->last_keyboard&KEY_PRESSED_OFFSET_V)==0)&&chassis_move.chassis_data_->keyboard&KEY_PRESSED_OFFSET_V)
	{
		HAL_Software_Reset();
	}
...
static void chassis_control_loop(chassis_move_t *chassis_move_control_loop)
{
    ...
    for (i = 0; i < 4; i++)
    {
        chassis_move_control_loop->motor_chassis[i].give_current = (int16_t)((chassis_move_control_loop->motor_speed_pid[i].out)+Feedforward(&feed,chassis_move_control_loop->motor_chassis[i].speed_set)) ;
    }
}
