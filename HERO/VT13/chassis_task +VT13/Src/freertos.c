...
osThreadId chassis_to_gimbal_task_handle;
...
void MX_FREERTOS_Init(void) {
  ...
  /* add threads, ... */
    ...
    referee_usart_task_handle = osThreadCreate(osThread(CAN_TASK), NULL);
	
    osThreadDef(buzr, buzzer_effects_task, osPriorityBelowNormal, 0, 512);
	buzzerHandle = osThreadCreate(osThread(buzr), NULL);

    osThreadDef(chassis_to_gimbal, chassis_to_gimbal_task, osPriorityNormal, 0, 256);
    chassis_to_gimbal_task_handle = osThreadCreate(osThread(chassis_to_gimbal), NULL);
