...
void buzzer_effects_task(void const *argument)
{
	...
	buzzer_control.sound_effect = SOUND_STOP;  
	...
	buzzer_control.sound_effect = SOUND_SYSTEM_START_BEEP;
	...
	for (;;)
	{
		...
		if (buzzer_control.work == TRUE)
		{
			...
			case SOUND_D_CONTINUE:
				buzzer_is_busy = TRUE;
				buzzer_drv_on(4, 10000);    osDelay(100);
				buzzer_drv_off();           osDelay(50);
				break;

			case SOUND_D_B_B_SHORT:
				buzzer_control.sound_effect = SOUND_STOP;
				buzzer_is_busy = TRUE;
				buzzer_drv_on(4, 10000);	osDelay(70);
				buzzer_drv_off();			osDelay(70);
		...
		else if (buzzer_control.sound_effect != SOUND_STOP)
		{
			buzzer_control.sound_effect = SOUND_STOP;
			buzzer_drv_off();
			buzzer_is_busy = FALSE;
