...
void chassis_rc_to_control_vector(gimbal_control_t *gimbal_control_set,chassis_data_t *chassis_data)
{
    ...
    rc_deadband_limit(gimbal_control_set->gimbal_rc_ctrl->rc.ch_0, vx_channel, CHASSIS_RC_DEADLINE);
    rc_deadband_limit(gimbal_control_set->gimbal_rc_ctrl->rc.ch_1, vy_channel, CHASSIS_RC_DEADLINE);
	rc_deadband_limit(gimbal_control_set->gimbal_rc_ctrl->rc.ch_2, yaw_channel, RC_DEADBAND1);
	rc_deadband_limit(gimbal_control_set->gimbal_rc_ctrl->rc.ch_3, pitch_channel, RC_DEADBAND);
...
//	chassis_data->shoot_mode=gimbal_control_set->gimbal_rc_ctrl->mouse.press_l<<1|shoot_control.shoot_mode|aimflag<<2;
	chassis_data->shoot_mode=gimbal_control_set->gimbal_rc_ctrl->rc.fn_2;
