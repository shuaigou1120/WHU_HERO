...
//	}
	if((shoot_control.angle-shoot_control.set_angle)>2000)
	{
		anti_jamming_flag=1;
	}
	else
...
//if((shoot_control.last_press_l==0&&shoot_control.press_l)&&shoot_control.shoot_friction_mode==1&&!toe_is_error(TRIGGER_MOTOR_TOE)&&anti_jamming_flag==0)
	if((shoot_control.last_press_l==3&&shoot_control.press_l==1)&&anti_jamming_flag==0)
	{
		shoot_control.set_angle+=1140;//3508减速比1:19
		shoot_mode=2;
	}
	
	else if (shoot_control.selfaim_mode==2)
