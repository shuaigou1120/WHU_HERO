...
uint8_t chassis_flag=0,aimflag=0,turnflag=0,videoflag=0,tuidanflag=0;
//底盘控制所有相关数据
...
//初始化video电机pid
    ...
    init->gimbal_video_motor.motor_angle_set=init->gimbal_video_motor.gimbal_motor_measure->angle;
...
static void gimbal_feedback_update(gimbal_control_t *feedback_update)
{
    ...
    //云台数据更新
	feedback_update->gimbal_pitch_motor.relative_angle = (-feedback_update->gimbal_pitch_motor.gimbal_motor_measure->angle-148)*10/2083*PI/180;
...
static void gimbal_set_control(gimbal_control_t *set_control)
{
	...
	static  int16_t pitch_mouse = 0;
	static  int16_t video_channel = 0;
	static  int16_t video_mouse = 0;
	static  int16_t a=0;
	static  int16_t b=0;
	//PITCH电机获取遥控器角度值
	rc_deadband_limit(set_control->gimbal_rc_ctrl->rc.ch[PITCH_CHANNEL], pitch_channel, RC_DEADBAND);
//	rc_deadband_limit(set_control->gimbal_rc_ctrl->mouse.z, pitch_mouse, PITCH_DEADBAND   );
	set_control->gimbal_pitch_motor.motor_angle_set+=pitch_channel*PITCH_RC_SEN+set_control->gimbal_rc_ctrl->mouse.z*PITCH_MOUSE_SEN;
	
//   图传电机的遥控器接受数据	
	rc_deadband_limit(set_control->gimbal_rc_ctrl->rc.ch[4], video_channel, RC_DEADBAND);
	rc_deadband_limit(set_control->gimbal_rc_ctrl->mouse.y, video_mouse, PITCH_DEADBAND);
//	a=b;
...
set_control->gimbal_video_motor.absolute_angle_set+=video_channel*0.01+video_mouse*0.5;
...
void chassis_rc_to_control_vector(gimbal_control_t *gimbal_control_set,chassis_data_t *chassis_data)
{
	...
	rc_deadband_limit(gimbal_control_set->gimbal_rc_ctrl->mouse.x, yaw_mouse, YAW_DEADBAND);
	rc_deadband_limit(gimbal_control_set->gimbal_rc_ctrl->rc.ch[YAW_CHANNEL], yaw_channel, RC_DEADBAND1);
...
//			}
		...
		else if(gimbal_control_set->keyboard&KEY_PRESSED_OFFSET_V&&(gimbal_control_set->lastkeyboard&KEY_PRESSED_OFFSET_V)==0)
		{
			chassis_flag=2;
		}
//	}
...
//	chassis_data->shoot_mode=gimbal_control_set->gimbal_rc_ctrl->mouse.press_l<<1|shoot_control.shoot_mode;
	chassis_data->shoot_mode=gimbal_control_set->gimbal_rc_ctrl->mouse.press_l<<1;
//	chassis_data->tuidan_mode=(gimbal_control_set->keyboard&KEY_PRESSED_OFFSET_E&&(gimbal_control_set->lastkeyboard&KEY_PRESSED_OFFSET_E)==0);
	chassis_data->tuidan_mode=(gimbal_control_set->keyboard==KEY_PRESSED_OFFSET_E);
...
//	chassis_data->yawangle_set=yaw_angle_limit_func(chassis_data->yawangle_set-yaw_channel*YAW_RC_SEN);
	...
	{
		chassis_data->yawangle_set-=yaw_channel*YAW_RC_SEN+yaw_mouse*YAW_MOUSE_SEN;
	}
...
void servo_control(gimbal_control_t *servo)
{
	...
	last_press=press;
	press=servo->keyboard&KEY_PRESSED_OFFSET_B;
	if((last_press==0)&&press)
	{
	servoflag=!servoflag;
	}
	if(servoflag==1)
