...
static uint8_t sbus_rx_buf[2][SBUS_RX_BUF_NUM1];
...
void USART3_IRQHandler(void)
{
    ...
    else if(USART3->SR & UART_FLAG_IDLE)
    {
        ...
        if ((hdma_usart3_rx.Instance->CR & DMA_SxCR_CT) == RESET)
        {
            ...
            this_time_rx_len = SBUS_RX_BUF_NUM1 - hdma_usart3_rx.Instance->NDTR;

            //reset set_data_lenght
            //重新设定数据长度
            hdma_usart3_rx.Instance->NDTR = SBUS_RX_BUF_NUM1;
			...
			{
				if((verify_CRC16_check_sum(sbus_rx_buf[0], RC_FRAME_LENGTH1)))
				{
...
//                detect_hook(DBUS_TOE);
        ...
        else
        {
			...
			{
				if((verify_CRC16_check_sum(sbus_rx_buf[1], RC_FRAME_LENGTH1)))
				{
				if(this_time_rx_len == RC_FRAME_LENGTH1)
				{
                //处理遥控器数据
