...
float INS_angle[3] = {0, 0, 0};      //euler angle, unit rad.欧拉角 单位 rad
...
void INS_Task(void)
{
    ...
    if ((count % 1) == 0)
    {
        ...
        INS.Gyro[X] = BMI088.Gyro[X]+gyro_offset[X];
        INS.Gyro[Y] = BMI088.Gyro[Y]+gyro_offset[Y];
        INS.Gyro[Z] = BMI088.Gyro[Z]+gyro_offset[Z];
