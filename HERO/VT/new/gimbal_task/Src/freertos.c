...
osThreadId calibrationHandle;

//osThreadId vofa_task_handle;
/* USER CODE END PTD */
...
void MX_FREERTOS_Init(void) {
  ...
  /* add threads, ... */
    ...
    Self_aim_task_handle = osThreadCreate(osThread(SELF_AIM_task), NULL);


    osThreadDef(vofa, vofa_task, osPriorityBelowNormal, 0, 512);
    vofaHandle = osThreadCreate(osThread(vofa), NULL);
	
    osThreadDef(buzr, buzzer_effects_task, osPriorityNormal, 0, 512);
	buzzerHandle = osThreadCreate(osThread(buzr), NULL);
	
	osThreadDef(calibration, calibration_task, osPriorityNormal, 0, 512);
	calibrationHandle = osThreadCreate(osThread(calibration), NULL);
	
//    osThreadDef(VOFA_task, vofa_task, osPriorityBelowNormal, 0, 512);
//    vofa_task_handle = osThreadCreate(osThread(VOFA_task), NULL);
