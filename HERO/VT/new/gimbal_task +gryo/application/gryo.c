...
void gryo_init( gryo_get_t *init)
{
	...
	init->init_offset=init->gryo_get->yaw;
...
void gryo_feedback(gryo_get_t *feedback)
{
	...
	if(feedback->init_flag==0)
	{
		feedback->yaw=feedback->gryo_get->yaw-feedback->init_offset;
		feedback->init_flag=1;
	}

	
	feedback->last_yaw=feedback->yaw;
	feedback->yaw=feedback->gryo_get->yaw-feedback->init_offset;
	...
	// 处理 yaw 360° 循环问题
	if(feedback->yaw - feedback->last_yaw >=180.0f)
	{
	...
	feedback->all_yaw = 360.0f * feedback->YawRoundNumber + feedback->yaw;
}
