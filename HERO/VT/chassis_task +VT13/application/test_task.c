...
//    error_list_test_local = get_error_list_point();
    ...
    while(1)
    {
		...
		if(chassis_move.chassis_data_->shoot_flag==1)
		{
			ui_MODE_mode_FIRE->color = 1;
			
		}
		
		//进退弹标识
		if(shoot_control.shoot_control_data->ejection_flag==1)
		{
			ui_MODE_mode_unload->color=1;
		}
		
		//底盘相对位置改变
		if(chassis_move.chassis_relative_angle<=0)
		{
			ui_MODE_mode_direction->start_angle=-(chassis_move.chassis_relative_angle)+35;
			if(chassis_move.chassis_relative_angle<=-35)
			{
			ui_MODE_mode_direction->end_angle =-(chassis_move.chassis_relative_angle)-35;
			}
			else
			{
		    ui_MODE_mode_direction->end_angle =-(chassis_move.chassis_relative_angle)+325;
			}
		}
		...
		if(chassis_move.chassis_relative_angle>=0)
		{
			...
			if(chassis_move.chassis_relative_angle>=35)
			{
			ui_MODE_mode_direction->start_angle=-(chassis_move.chassis_relative_angle)+395;
			}
			else
			{
		    ui_MODE_mode_direction->start_angle=-(chassis_move.chassis_relative_angle)+35;
